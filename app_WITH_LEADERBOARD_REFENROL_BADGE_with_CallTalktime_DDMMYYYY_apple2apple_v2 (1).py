...
# ===================== Apple to Apple comparison (Performance) =====================
def _apple_to_apple_pill(df_f,
                         deal_stage_col="Deal Stage",
                         create_col="Create Date",
                         pay_col="Payment Received Date",
                         source_col="JetLearn Deal Source",
                         country_col="Country",
                         counsellor_col=None):
    import streamlit as st
    import pandas as pd
    ...
    st.subheader("Apple to Apple comparison")

    # --- Utilities: parse dates day-first like rest of the app ---
    def _to_dt(x):
        return pd.to_datetime(x, errors="coerce", dayfirst=True)

    # Ensure trimmed columns (app already trims globally; safe to re-guard)
    df = df_f.copy()
    df.columns = [c.strip() for c in df.columns]

    # Default counsellor col resolution if present in dataset
    if counsellor_col is None:
        for cand in ["Academic Counsellor", "Student/Academic Counsellor", "Counsellor", "Sales Owner"]:
            if cand in df.columns:
                counsellor_col = cand
                break

    # Drop invalid deals if the stage column exists and contains the invalid marker
    if deal_stage_col in df.columns:
        mask_valid = ~df[deal_stage_col].astype(str).str.contains("1.2 Invalid", case=False, na=False)
        df = df[mask_valid]

    # Date columns
    if create_col in df.columns:
        df["_create"] = _to_dt(df[create_col])
    else:
        df["_create"] = pd.NaT
    if pay_col in df.columns:
        df["_pay"] = _to_dt(df[pay_col])
    else:
        df["_pay"] = pd.NaT

    # Group By selector (common)
    DIM_MAP = {
        "JetLearn Deal Source": source_col if source_col in df.columns else None,
        "Country": country_col if country_col in df.columns else None,
        "Academic Counsellor": counsellor_col if counsellor_col in df.columns else None,
    }
    dim_choice = st.selectbox("Group By (first column)", list(DIM_MAP.keys()), index=0)
    dim_col = DIM_MAP[dim_choice]
    if dim_col is None:
        st.warning(f"Column for '{dim_choice}' was not found in the dataset.")
        return

    # Canonical ordering for Deal Source (customize as needed)
    canonical_sources = [
        "Affinity","PM Search","PM Social","Organic","Events","Referral","Sibling",
        "Inbound","Outbound","Website","Bot","Email","WhatsApp"
    ]

    # Metric selection (common)
    metric_opts = ["Deals Created","Enrolments"]
    metrics = st.multiselect("Select metrics (apply to both A & B)", metric_opts, default=metric_opts)

    # Two columns layout
    colA, colB = st.columns(2)

    def window_controls(col, label_prefix):
        with col:
            st.markdown(f"**Window {label_prefix}**")
            mode = st.radio(f"Mode — {label_prefix}", ["MTD","Cohort"], horizontal=True, index=0, key=f"mode_{label_prefix}")
            a_dt = st.date_input(f"Date range — {label_prefix}", key=f"dr_{label_prefix}", value=None)
            # Normalize tuple/date to start/end
            if isinstance(a_dt, tuple) and len(a_dt) == 2:
                start_date, end_date = a_dt
            else:
                # if single date picked, treat as same-day range; else fall back to min/max present
                series = df["_create"].dropna()
                if series.empty:
                    series = pd.Series([pd.Timestamp("2020-01-01"), pd.Timestamp("2025-12-31")])
                start_date = series.min().date()
                end_date = series.max().date()
            # Optional filter within the same dimension
            distinct_vals = (
                df[dim_col].dropna().astype(str).unique().tolist()
            )
            # Nice ordering for sources when applicable
            if dim_col == source_col:
                # Put canonical first then the rest
                rest = [x for x in distinct_vals if x not in canonical_sources]
                ordered = [x for x in canonical_sources if x in distinct_vals] + sorted(rest)
                distinct_vals = ordered
            inc = st.multiselect(f"Include specific {dim_choice} — {label_prefix} (optional)", distinct_vals, default=distinct_vals, key=f"inc_{label_prefix}")
            return {"mode": mode, "start": pd.Timestamp(start_date), "end": pd.Timestamp(end_date), "include": set(map(str, inc))}

    A = window_controls(colA, "A")
    B = window_controls(colB, "B")

    # Compute metric frames
    def compute_frame(win):
        d = df.copy()
        # apply include filter on dimension
        d = d[d[dim_col].astype(str).isin(win["include"])]
        # date windows
        s, e = win["start"].normalize(), (win["end"] + pd.Timedelta(days=1)).normalize() - pd.Timedelta(seconds=1)

        out = {dim_col: d[dim_col].astype(str)}
        grp = d.assign(**out)

        res = pd.DataFrame({dim_col: grp[dim_col].astype(str)})
        res = res.groupby(dim_col, dropna=False).size().reset_index().iloc[0:0]  # empty starter with dim column

        if "Deals Created" in metrics:
            if win["mode"] == "MTD":
                mask = d["_create"].between(s, e) & d["_create"].notna()
            else:  # Cohort by create
                mask = d["_create"].between(s, e) & d["_create"].notna()
            created = d.loc[mask].groupby(dim_col)[dim_col].size().rename("Deals Created")
            res = res.merge(created, left_on=dim_col, right_index=True, how="outer")

        if "Enrolments" in metrics:
            if win["mode"] == "MTD":
                # MTD definition consistent with app: create in range AND pay in range
                mask = d["_create"].between(s, e) & d["_pay"].between(s, e)
            else:  # Cohort by payment date
                mask = d["_pay"].between(s, e)
            paid = d.loc[mask].groupby(dim_col)[dim_col].size().rename("Enrolments")
            res = res.merge(paid, left_on=dim_col, right_index=True, how="outer")
        ...
        res = res.fillna(0).reset_index(drop=True)
        # Ensure all included keys appear, even if zero
        all_keys = sorted(list(win["include"]))
        ensure = pd.DataFrame({dim_col: all_keys})
        res = ensure.merge(res, on=dim_col, how="left").fillna(0)
        return res

    dfA = compute_frame(A)
    dfB = compute_frame(B)

    # Merge A vs B & compute deltas
    sufA = " (A)"
    sufB = " (B)"
    key = dim_col
    merged = pd.merge(dfA, dfB, on=key, how="outer", suffixes=(sufA, sufB)).fillna(0)

    # Build delta cols
    for m in metrics:
        a = f"{m}{sufA}" if f"{m}{sufA}" in merged.columns else m
        b = f"{m}{sufB}" if f"{m}{sufB}" in merged.columns else m
        merged[f"Δ {m} (B−A)"] = merged[b] - merged[a]
        # %Δ with safe divide
        merged[f"%Δ {m}"] = np.where(merged[a] == 0, np.nan, (merged[b] - merged[a]) / merged[a] * 100.0)

    # Sorting: by first selected metric Δ descending by default
    if metrics:
        merged = merged.sort_values(by=[f"Δ {metrics[0]} (B−A)"], ascending=False)

    # KPI header totals
    st.markdown("**Totals**")
    kpi_cols = st.columns(max(1, len(metrics)))
    for i, m in enumerate(metrics):
        with kpi_cols[i]:
            a_tot = merged.get(f"{m}{sufA}", pd.Series([0])).sum()
            b_tot = merged.get(f"{m}{sufB}", pd.Series([0])).sum()
            delta = b_tot - a_tot
            pct = None if a_tot == 0 else (delta / a_tot * 100.0)
            arrow = "↑" if delta > 0 else ("↓" if delta < 0 else "→")
            st.metric(label=m, value=int(b_tot), delta=f"{int(delta)} ({'' if pct is None else f'{pct:.1f}% '}{arrow})")

    # Show table
    # Order columns: key, for each metric -> A, B, Δ, %Δ
    cols = [key]
    for m in metrics:
        cols += [f"{m}{sufA}", f"{m}{sufB}", f"Δ {m} (B−A)", f"%Δ {m}"]
    merged_display = merged[cols].copy()
    # Format %
    for m in metrics:
        c = f"%Δ {m}"
        merged_display[c] = merged_display[c].map(lambda x: "" if pd.isna(x) else f"{x:.1f}%")
    st.dataframe(merged_display, use_container_width=True)

    # Badges
    st.caption(
        f"**A**: {A['mode']} | {A['start'].date()} → {A['end'].date()}  •  "
        f"**B**: {B['mode']} | {B['start'].date()} → {B['end'].date()}  •  "
        f"Grouped by **{dim_choice}**  •  Metrics: {', '.join(metrics)}"
    )
# =================== End Apple to Apple comparison ====================

# Handler fallback if not matched above
try:
    if perf_view == "Apple to Apple comparison":
        _apple_to_apple_pill(df_f, 
                             deal_stage_col="Deal Stage",
                             create_col="Create Date",
                             pay_col="Payment Received Date",
                             source_col="JetLearn Deal Source",
    ...
    import streamlit as st
    st.error(f"Apple to Apple comparison error: {_e}")
