...
def _render_call_talktime_report():
    ...
    st.caption("Date is parsed as DDMMYYYY (e.g., 25092025) and **DD:MM:YYYY** (e.g., 25:09:2025). Time supports formats like '18' (6 PM), '5;26;00 PM', '17:45:30'. Call Duration sums HH:MM:SS or HH;MM;SS.")
...
st.markdown("#### Filters")
uicol1, uicol2 = st.columns([2,1])
with uicol1:
    st.caption("Enter dates in **DD:MM:YYYY** (same as your sheet). Other formats like DD/MM/YYYY also work.")
with uicol2:
    fmt_help = st.toggle("Use text date inputs (DD:MM:YYYY)", value=True, key="ctt_text_dates")

if fmt_help:
    # Text inputs that accept DD:MM:YYYY (and friends)
    def fmt_d(d): 
        try:
    ...
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        d_start_str = st.text_input("Start date (DD:MM:YYYY)", value=fmt_d(min_d), key="ctt_start_d_str")
    with c2:
        d_end_str = st.text_input("End date (DD:MM:YYYY)", value=fmt_d(max_d), key="ctt_end_d_str")
    with c3:
    ...
    def parse_text_date(s):
        d = _ctt_parse_date_ddmmyyyy(s)
        return d or min_d

    d_start = parse_text_date(d_start_str)
    d_end   = parse_text_date(d_end_str)
else:
    c1, c2, c3, c4 = st.columns(4)
    with c1:
...
if isinstance(d_start, dt.date): 
    pass
else:
    d_start = dt.datetime.strptime(str(d_start), "%Y-%m-%d").date()
if isinstance(d_end, dt.date): 
    pass
else:
    d_end = dt.datetime.strptime(str(d_end), "%Y-%m-%d").date()
...
def _render_performance_quick_view(
    ...
    *,
    ...
    # Helpers
    def _section_tables(df_source: pd.DataFrame, title: str, overall: bool, label_suffix: str):
        with st.container():
            st.markdown(title)
            ...
            with c1:
                if overall:
                    st.dataframe(
                        pd.DataFrame({f"Deals Created {label_suffix}":[int(df_source['Deals_Created'].sum())]}),
                        use_container_width=True, hide_index=True
                    )
                else:
                    bx1 = (
                        df_source.groupby("__src", dropna=False)["Deals_Created"]
                           .sum()
                           .reset_index()
                           .rename(columns={"__src":"JetLearn Deal Source", "Deals_Created": f"Deals Created {label_suffix}"})
                    )
            ...
            with c2:
                if overall:
                    ...
                    bx2 = (
                        df_source.groupby("__src", dropna=False)[["First_Cal","Cal_Resched","Cal_Done"]]
                           .sum()
                           .reset_index()
            ...
            with c3:
                if overall:
                    st.dataframe(
                        pd.DataFrame({f"Enrolments {label_suffix}":[int(df_source['Enrolments'].sum())]}),
                        use_container_width=True, hide_index=True
                    )
                else:
                    bx3 = (
                        df_source.groupby("__src", dropna=False)["Enrolments"]
                           .sum()
                           .reset_index()
                           .rename(columns={"__src":"JetLearn Deal Source", "Enrolments": f"Enrolments {label_suffix}"})
                    )
                    st.dataframe(bx3, use_container_width=True, hide_index=True)

    tab_by_src, tab_overall = st.tabs(["By Source", "Overall"])
    ...
    d_r["Cal_Resched"]   = m_resch.astype(int) if isinstance(m_resch, pd.Series) else m_resch
    d_r["Cal_Done"]      = m_done.astype(int) if isinstance(m_done, pd.Series) else m_done
    d_r["Enrolments"]    = enrol_mask.astype(int)

    with tab_by_src:
        _section_tables(d_y, "## Yesterday", overall=False, label_suffix="(yesterday)")
        st.divider()
        _section_tables(d_t, "## Today", overall=False, label_suffix="(today)")
        st.divider()
        _section_tables(d_r, "## Range", overall=False, label_suffix="(range)")

    with tab_overall:
        _section_tables(d_y, "## Yesterday — Overall", overall=True, label_suffix="(yesterday)")
        st.divider()
        _section_tables(d_t, "## Today — Overall", overall=True, label_suffix="(today)")
        st.divider()
        _section_tables(d_r, "## Range — Overall", overall=True, label_suffix="(range)")
