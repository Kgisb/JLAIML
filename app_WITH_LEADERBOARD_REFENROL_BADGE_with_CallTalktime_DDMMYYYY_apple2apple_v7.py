...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail","Leaderboard", "Call Talk-time Report", "Apple-to-Apple Comparison"],
...
# ==============================
# Apple-to-Apple Comparison (Performance pill)
# ==============================
import streamlit as _st_ata
import pandas as _pd_ata
import numpy as _np_ata
import datetime as _dt_ata
import re as _re_ata
from calendar import monthrange as _monthrange_ata

_REF_PAT_ATA = _re_ata.compile(r"referr?al", _re_ata.I)

def _ata_month_bounds(d):
    return _dt_ata.date(d.year, d.month, 1), _dt_ata.date(d.year, d.month, _monthrange_ata(d.year, d.month)[1])

def _ata_last_month_bounds(today):
    first = _dt_ata.date(today.year, today.month, 1)
    end_prev = first - _dt_ata.timedelta(days=1)
    return _dt_ata.date(end_prev.year, end_prev.month, 1), end_prev

def _ata_pick(df, variants):
    # reuse app's find_col if available
    try:
        return find_col(df, variants)
    except Exception:
        # fallback
        for v in variants:
...
def _ata_to_datetime(s):
    try:
        # reuse app's coerce_datetime if present
        return coerce_datetime(s)
    except Exception:
        s2 = _pd_ata.to_datetime(s, errors="coerce", dayfirst=True)
        mask_num = s2.isna() & _pd_ata.to_numeric(s, errors="coerce").notna()
        if mask_num.any():
            v = _pd_ata.to_numeric(s[mask_num], errors="coerce")
            s2.loc[mask_num] = _pd_ata.to_datetime(v, unit="s", errors="coerce")
            mask_ms = s2.loc[mask_num].isna()
            if mask_ms.any():
                s2.loc[mask_ms.index] = _pd_ata.to_datetime(v, unit="ms", errors="coerce")
        return s2

def _ata_window_ui(prefix):
    today = _dt_ata.date.today()
    c1, c2, c3 = _st_ata.columns([1.0,1.0,1.0])
    with c1:
        mode = _st_ata.radio(f"{prefix} — Counting mode", ["MTD","Cohort"], index=0, horizontal=True, key=f"ata_mode_{prefix}")
    with c2:
        scope = _st_ata.radio(f"{prefix} — Date scope", ["This month","Last month","Custom"], index=0, horizontal=True, key=f"ata_scope_{prefix}")
    with c3:
        gran = _st_ata.radio(f"{prefix} — Granularity", ["Day","Week","Month"], index=0, horizontal=True, key=f"ata_gran_{prefix}")

    if scope == "This month":
        start, end = _ata_month_bounds(today)
    elif scope == "Last month":
        start, end = _ata_last_month_bounds(today)
    else:
        c4, c5 = _st_ata.columns(2)
        with c4: start = _st_ata.date_input(f"{prefix} Start", value=_ata_month_bounds(today)[0], key=f"ata_start_{prefix}")
        with c5: end   = _st_ata.date_input(f"{prefix} End",   value=_ata_month_bounds(today)[1], key=f"ata_end_{prefix}")
        if end < start:
            _st_ata.error(f"{prefix}: End date cannot be before start date.")
    return mode, scope, gran, start, end

def _ata_prepare_df():
    # Use the app's data pipeline exactly
    data_src = _st_ata.session_state.get("data_src", globals().get("DEFAULT_DATA_PATH", "Master_sheet-DB.csv"))
    df = load_csv(data_src)  # app's loader
    dealstage_col = find_col(df, ["Deal Stage","Deal stage","Stage","Deal Status","Stage Name","Deal Stage Name"])
    df, _removed = exclude_invalid_deals(df, dealstage_col)
    return df

def _ata_apply_global_filters(df):
    counsellor_col = _ata_pick(df, ["Student/Academic Counsellor","Academic Counsellor","Counsellor","Counselor","Student/Academic Counselor"])
    country_col    = _ata_pick(df, ["Country","Country Name"])
    source_col     = _ata_pick(df, ["JetLearn Deal Source","Deal Source","Source"])
    pipeline_col   = _ata_pick(df, ["Pipeline"])

    # Use app's existing UI/layout if available; else render here
    try:
        # If the app already created these selections, just re-use filtered df_f
        if "df_f" in globals():
            return globals()["df_f"], counsellor_col, country_col, source_col, pipeline_col
    except Exception:
        pass

    # Lightweight fallback filters (won't interfere with app's own)
    c1, c2, c3, c4 = _st_ata.columns([1,1,1,1])
    with c1:
        sel_cons = _st_ata.multiselect("Counsellor", sorted(df[counsellor_col].dropna().astype(str).unique().tolist()) if counsellor_col else [], placeholder="All")
    with c2:
        sel_cnt  = _st_ata.multiselect("Country", sorted(df[country_col].dropna().astype(str).unique().tolist()) if country_col else [], placeholder="All")
    with c3:
        sel_src  = _st_ata.multiselect("Deal Source", sorted(df[source_col].dropna().astype(str).unique().tolist()) if source_col else [], placeholder="All")
    with c4:
        sel_pipe = _st_ata.multiselect("Pipeline", sorted(df[pipeline_col].dropna().astype(str).unique().tolist()) if pipeline_col else [], placeholder="All")

    # Prefer app's apply_filters if available
    try:
        f = apply_filters(df, counsellor_col, country_col, source_col, sel_cons, sel_cnt, sel_src)
    except Exception:
        f = df.copy()
        if counsellor_col and sel_cons and "All" not in sel_cons:
            f = f[f[counsellor_col].astype(str).isin(sel_cons)]
        if country_col and sel_cnt and "All" not in sel_cnt:
            f = f[f[country_col].astype(str).isin(sel_cnt)]
        if source_col and sel_src and "All" not in sel_src:
            f = f[f[source_col].astype(str).isin(sel_src)]
    # Track filter (Both / AI Coding / Math) if available
    try:
        track = globals().get("track", None) or _st_ata.session_state.get("track", None)
        if track and track != "Both" and pipeline_col and pipeline_col in f.columns:
            f = f[f[pipeline_col].astype(str).str.contains(track, case=False, na=False)]
    except Exception:
        pass

    return f, counsellor_col, country_col, source_col, pipeline_col

def _ata_metric_totals(df, metric, start, end):
    src_col   = _ata_pick(df, ["JetLearn Deal Source","Deal Source","Source"])
    metric_map = {
        "Leads (Creates)": ["Create Date","Create date","Create_Date","Created At","Deal Create Date","Created On"],
        "Enrolments": ["Payment Received Date","Payment Received date","Payment_Received_Date","Payment Date","Paid At","Enrolment Date","Payment Received Date "],
        "First Cal Scheduled": ["First Calibration Scheduled Date","First Calibration","First Cal Scheduled"],
        "Cal Rescheduled": ["Calibration Rescheduled Date","Cal Rescheduled","Cal Reschedule","Cal Re-scheduled"],
        "Cal Done": ["Calibration Done Date","Cal Done","Trial Done","Trial Completed Date"],
        "Referral Deals": ["Create Date","Create date","Create_Date","Created At","Deal Create Date","Created On"],
        "Referral Enrolments": ["Payment Received Date","Payment Received date","Payment_Received_Date","Payment Date","Paid At","Enrolment Date","Payment Received Date "],
    }
    date_col = _ata_pick(df, metric_map.get(metric, []))
    if src_col is None or date_col is None:
        return _pd_ata.DataFrame(columns=["source","value"])
    s = _ata_to_datetime(df[date_col])
    mask = s.notna() & (s.dt.date >= start) & (s.dt.date <= end)
    dfx = df.loc[mask].copy()

    if metric.startswith("Referral"):
        dfx = dfx[dfx[src_col].astype(str).str.contains(_REF_PAT_ATA, na=False)]
        if metric == "Referral Enrolments":
            pay_col = _ata_pick(dfx, metric_map["Enrolments"])
            if pay_col is not None:
                dfx = dfx[_ata_to_datetime(dfx[pay_col]).notna()]
            else:
                dfx = dfx.iloc[0:0]

    out = (dfx.groupby(src_col, dropna=False).size().reset_index(name="value").rename(columns={src_col:"source"}))
    return out

def _ata_render():
    label = "Apple-to-Apple Comparison"
    # Respect the app's nav; only render when selected
    _lbl = globals().get('view') or globals().get('sub') or _st_ata.session_state.get('nav_sub') or _st_ata.session_state.get('view')
    if _lbl != label:
        return

    _st_ata.subheader("Performance — Apple-to-Apple Comparison")
    _st_ata.caption("Compare two windows (A vs B) with MTD/Cohort and Day/Week/Month granularity.")

    df0 = _ata_prepare_df()
    f, counsellor_col, country_col, source_col, pipeline_col = _ata_apply_global_filters(df0)
    if f is None or f.empty:
        _st_ata.info("No rows after filters.")
        return

    # Metric & source selection
    c0, = _st_ata.columns([1])
    with c0:
        metric = _st_ata.selectbox("Metric", ["Leads (Creates)","Enrolments","First Cal Scheduled","Cal Rescheduled","Cal Done","Referral Deals","Referral Enrolments"], index=0)
    # Optional per-window data range pre-filters (apply BEFORE A/B windows are computed)
    with _st_ata.expander("Data Range A (optional pre-filter)", expanded=False):
        _whichA = _st_ata.selectbox(
            "Apply range on (A)",
            ["Create Date","Payment Received Date","First Calibration Scheduled Date","Calibration Rescheduled Date","Calibration Done Date"],
            index=0,
            key="ata_which_A",
            help="This filters the dataset for Window A BEFORE the A window is applied."
        )
        ca1, ca2 = _st_ata.columns(2)
        with ca1:
            _drA_start = _st_ata.date_input("Start (A)", value=_dt_ata.date.today().replace(day=1), key="ata_drA_start")
        with ca2:
            _drA_end   = _st_ata.date_input("End (A)",   value=_dt_ata.date.today(), key="ata_drA_end")
        _enableA = _st_ata.checkbox("Enable date pre-filter for A", value=False, key="ata_drA_enable")
    with _st_ata.expander("Data Range B (optional pre-filter)", expanded=False):
        _whichB = _st_ata.selectbox(
            "Apply range on (B)",
            ["Create Date","Payment Received Date","First Calibration Scheduled Date","Calibration Rescheduled Date","Calibration Done Date"],
            index=0,
            key="ata_which_B",
            help="This filters the dataset for Window B BEFORE the B window is applied."
        )
        cb1, cb2 = _st_ata.columns(2)
        with cb1:
            _drB_start = _st_ata.date_input("Start (B)", value=_dt_ata.date.today().replace(day=1), key="ata_drB_start")
        with cb2:
            _drB_end   = _st_ata.date_input("End (B)",   value=_dt_ata.date.today(), key="ata_drB_end")
        _enableB = _st_ata.checkbox("Enable date pre-filter for B", value=False, key="ata_drB_enable")
    with _st_ata.expander("Data Range (optional pre-filter)", expanded=False):
        _which = _st_ata.selectbox(
            "Apply range on",
            ["Create Date","Payment Received Date","First Calibration Scheduled Date","Calibration Rescheduled Date","Calibration Done Date"],
            index=0,
            help="This pre-filters the dataset before computing A vs B."
        )
        cdr1, cdr2 = _st_ata.columns(2)
        with cdr1:
            _dr_start = _st_ata.date_input("Start", value=_dt_ata.date.today().replace(day=1), key="ata_dr_start")
        with cdr2:
            _dr_end   = _st_ata.date_input("End",   value=_dt_ata.date.today(), key="ata_dr_end")
        _enable = _st_ata.checkbox("Enable date pre-filter", value=False, key="ata_dr_enable")
    
    # If enabled, apply pre-filter on selected date column
    if _st_ata.session_state.get("ata_dr_enable"):
        _var_map = {
            "Create Date": ["Create Date","Create date","Create_Date","Created At","Deal Create Date","Created On"],
            "Payment Received Date": ["Payment Received Date","Payment Received date","Payment_Received_Date","Payment Date","Paid At","Enrolment Date","Payment Received Date "],
            "First Calibration Scheduled Date": ["First Calibration Scheduled Date","First Calibration","First Cal Scheduled"],
            "Calibration Rescheduled Date": ["Calibration Rescheduled Date","Cal Rescheduled","Cal Reschedule","Cal Re-scheduled"],
            "Calibration Done Date": ["Calibration Done Date","Cal Done","Trial Done","Trial Completed Date"],
        }
        _sel = _st_ata.session_state.get("ata_dr_enable") and _which
        if _sel:
            _col = _ata_pick(f, _var_map[_which])
            if _col:
                _ds = _ata_to_datetime(f[_col])
                _mask = _ds.notna() & (_ds.dt.date >= _st_ata.session_state.get("ata_dr_start")) & (_ds.dt.date <= _st_ata.session_state.get("ata_dr_end"))
                f = f.loc[_mask].copy()
            else:
                _st_ata.warning(f"Could not find column for '{_which}' in data.")

    _st_ata.markdown("### Time Windows")
    _st_ata.markdown("**Window A**")
    modeA, scopeA, granA, startA, endA = _ata_window_ui("A")
    _st_ata.markdown("**Window B**")
    modeB, scopeB, granB, startB, endB = _ata_window_ui("B")

    all_sources = sorted(f[source_col].dropna().astype(str).unique().tolist()) if source_col else []
    display_sources = _st_ata.multiselect("Show Sources", options=all_sources, default=all_sources[:12])

    # Build per-window prefiltered frames
    fA = f.copy()
    fB = f.copy()
    _var_map = {
        "Create Date": ["Create Date","Create date","Create_Date","Created At","Deal Create Date","Created On"],
    ...
    if _st_ata.session_state.get("ata_drA_enable"):
        _colA = _ata_pick(fA, _var_map[_st_ata.session_state.get("ata_which_A")])
        if _colA:
            _dsA = _ata_to_datetime(fA[_colA])
            _maskA = _dsA.notna() & (_dsA.dt.date >= _st_ata.session_state.get("ata_drA_start")) & (_dsA.dt.date <= _st_ata.session_state.get("ata_drA_end"))
            fA = fA.loc[_maskA].copy()
        else:
            _st_ata.warning("Could not resolve pre-filter column for A.")
    if _st_ata.session_state.get("ata_drB_enable"):
        _colB = _ata_pick(fB, _var_map[_st_ata.session_state.get("ata_which_B")])
        if _colB:
            _dsB = _ata_to_datetime(fB[_colB])
            _maskB = _dsB.notna() & (_dsB.dt.date >= _st_ata.session_state.get("ata_drB_start")) & (_dsB.dt.date <= _st_ata.session_state.get("ata_drB_end"))
            fB = fB.loc[_maskB].copy()
        else:
            _st_ata.warning("Could not resolve pre-filter column for B.")

    # Totals
    totA = _ata_metric_totals(fA, metric, startA, endA)
    totB = _ata_metric_totals(fB, metric, startB, endB)

    universe = display_sources if display_sources else sorted(set(totA["source"]).union(set(totB["source"])))
    base = _pd_ata.DataFrame({"source": universe})
    merged = base.merge(totA.rename(columns={"value":"A"}), on="source", how="left").merge(totB.rename(columns={"value":"B"}), on="source", how="left")
    merged["A"] = merged["A"].fillna(0).astype(int)
    merged["B"] = merged["B"].fillna(0).astype(int)
    merged["Δ"] = merged["B"] - merged["A"]
    merged["Δ%"] = _np_ata.where(merged["A"]>0, merged["Δ"]/merged["A"]*100.0, _np_ata.nan)

    _st_ata.markdown("### Source-wise KPIs")
    for _, r in merged.iterrows():
        c1, c2, c3, c4 = _st_ata.columns([2,1,1,1])
        with c1: _st_ata.markdown(f"**{r['source'] if _pd_ata.notna(r['source']) else '(Unknown)'}**")
        with c2: _st_ata.metric("A", int(r["A"]))
        with c3: _st_ata.metric("B", int(r["B"]))
        with c4: _st_ata.metric("Δ (B−A)", int(r["Δ"]), delta=("–" if _pd_ata.isna(r["Δ%"]) else f"{r['Δ%']:.1f}%"))

    with _st_ata.expander("Download Summary", expanded=True):
        _st_ata.dataframe(merged, use_container_width=True)
        _st_ata.download_button("⬇️ Download CSV", data=merged.to_csv(index=False).encode("utf-8"), file_name="apple_to_apple_summary.csv", mime="text/csv")

# Router: run when selected (late hook so it sees prior state)
try:
    _ata_render()
except Exception as _e:
    import streamlit as st
    st.error(f"Apple-to-Apple error: {_e}")
# Router: run only when Performance pill is selected\nif 'view' in globals() and view == \"Apple-to-Apple Comparison\":\n    _ata_render()\n
