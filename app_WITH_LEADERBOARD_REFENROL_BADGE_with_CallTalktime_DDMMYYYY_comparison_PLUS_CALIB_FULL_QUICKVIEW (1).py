...
def _render_performance_quick_view(
    ...
    *,
    ...
    _pay_in    = pay_col    if pay_col    in df_f.columns else _pick_col(df_f, [pay_col, "Payment Received Date", "Payment Received Date "], substr_candidates=["payment","enrolment","enrollment"])
    ...
    _ac_in     = _pick_col(df_f, ["Academic Counsellor","Student/Academic Counsellor","AC","Counsellor","Sales Owner","Lead Owner","Owner","Advisor","Agent"], substr_candidates=["counsellor","counselor","owner"])
    ...
    # ---- guards
    if _src_in is None:
        st.warning("Deal Source column not found; showing 'Unknown' for source.", icon="⚠️")
    if _ac_in is None:
        st.info("Academic Counsellor column not found; AC tab will show 'Unknown'.")
    if _country_in is None:
        st.info("Country column not found; Country tab will show 'Unknown'.")
    ...
    d["__src"]    = d[_src_in].fillna("Unknown").astype(str) if _src_in in d.columns else "Unknown"
    d["__ac"]     = d[_ac_in].fillna("Unknown").astype(str) if _ac_in in d.columns else "Unknown"
    d["__country"]= d[_country_in].fillna("Unknown").astype(str) if _country_in in d.columns else "Unknown"
    ...
    # Controls: allow user to switch whether "Today/Yesterday" use real calendar or latest data day
    use_latest = st.toggle("Use latest data day for Today/Yesterday defaults", value=True, help="Turn this on if your dataset is from a previous month/day so you can see non‑blank counts.")
    ...
    # ---- generic tables for (By Source / AC / Country / Overall)
