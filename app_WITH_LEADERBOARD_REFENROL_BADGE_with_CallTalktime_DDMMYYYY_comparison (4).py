...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail","Leaderboard", "Comparison", "Call Talk-time Report"],
...
def predict_chart_stacked(tbl: pd.DataFrame):
    ...
    if tbl is None or tbl.empty:
        return alt.Chart(pd.DataFrame({'Source':[], 'Metric':[], 'Value':[]})).mark_bar()
    value_cols = [c for c in tbl.columns if c != 'Source']
    melt = tbl.melt(id_vars=['Source'], value_vars=value_cols, var_name='Metric', value_name='Value')
    return (alt.Chart(melt)
            .mark_bar()
            .encode(x=alt.X('Source:N', sort='-y'),
                    y=alt.Y('Value:Q'),
                    color='Metric:N',
                    tooltip=['Source','Metric','Value'])
            .properties(height=380))
def _render_performance_comparison(df_f, create_col, pay_col, counsellor_col, country_col, source_col):
    import streamlit as st
    import pandas as pd
    ...
    st.caption("Compare metrics across two independently-filtered windows (A & B) with separate date ranges. "
               "Mode supports **MTD** (event-date scoped) or **Cohort** (Create Date cohort, outcomes cumulative).")
    
    # Defensive col resolution
    ...
    metrics = st.multiselect(
        "Metrics to compare", ["Deals Created","Enrollments"],
        default=["Enrollments","Deals Created"], key="cmp_metrics"
    )
    if not metrics:
    ...
    if not dim_opts:
        st.warning("No grouping dimensions available.")
        return
    ...
    with colA:
        ...
        date_a = st.date_input("Date range (A)", value=(date.today().replace(day=1), date.today()), key="cmp_date_a")
    with colB:
        st.write("### Window B")
        dims_b = st.multiselect("Group by (B)", dim_opts, default=[dim_opts[0]], key="cmp_dims_b")
        date_b = st.date_input("Date range (B)", value=(date.today().replace(day=1), date.today()), key="cmp_date_b")
    
    def _ensure_tuple_date(val):
        # streamlit returns either a single date or a tuple
        if isinstance(val, (list, tuple)) and len(val) == 2:
            return pd.to_datetime(val[0]), pd.to_datetime(val[1])
        # fallbacks: interpret as single date
        return pd.to_datetime(val), pd.to_datetime(val)
    
    a_start, a_end = _ensure_tuple_date(date_a)
    b_start, b_end = _ensure_tuple_date(date_b)
    if a_end < a_start: a_start, a_end = a_end, a_start
    if b_end < b_start: b_start, b_end = b_end, b_start
    
    def _agg(df, dims, start, end):
        # Build mask depending on mode for each metric
        out = {}
        if not dims: dims = []
        group_cols = []
        for dname in dims:
        ...
        for m in metrics:
            if m == "Deals Created":
                if mode == "MTD":
                    val = m_create
                    cnt = g.loc[val].groupby(group_cols, dropna=False).size()
                else:  # Cohort: deals created within cohort window
                ...
                else:  # Cohort: of cohort (create in window), how many ever enrolled (pay not null)
                    cohort = g.loc[m_create]
                    cnt = cohort.assign(_ok=cohort["_pay"].notna()).groupby(group_cols, dropna=False)["_ok"].sum(min_count=1)
    ...
    res_a, keys_a = _agg(d, dims_a, a_start, a_end)
    res_b, keys_b = _agg(d, dims_b, b_start, b_end)
    
    # Unified key set (outer join)
    # If keys differ, we outer-join on the intersection of column names present in both results
    on_cols = [c for c in res_a.columns if c in res_b.columns and c in ["Academic Counsellor","JetLearn Deal Source","Country","All"]]
    if not on_cols:
        # Different group keys: create a synthetic key to allow a Cartesian-like alignment on label
        # Create label columns
        def _label(df, keys):
            if keys:
                return df.assign(_KeyLabel=df[keys].astype(str).agg(" | ".join, axis=1))
            return df.assign(_KeyLabel="All")
        ra = _label(res_a, [k for k in ["Academic Counsellor","JetLearn Deal Source","Country","All"] if k in res_a.columns])
        rb = _label(res_b, [k for k in ["Academic Counsellor","JetLearn Deal Source","Country","All"] if k in res_b.columns])
        merged = pd.merge(ra, rb, on="_KeyLabel", how="outer", suffixes=(" (A)", " (B)"))
    else:
        merged = pd.merge(res_a, res_b, on=on_cols, how="outer", suffixes=(" (A)", " (B)"))
    ...
    for m in metrics:
        ...
        if colA in merged.columns and colB in merged.columns:
            merged[f"Δ {m} (B−A)"] = merged[colB].fillna(0) - merged[colA].fillna(0)
            with pd.option_context("mode.use_inf_as_na", True):
                denom = merged[colA].replace(0, pd.NA)
                merged[f"% Δ {m} (vs A)"] = (merged[colB].fillna(0) / denom * 100).round(1)
    
    # Order columns: keys, A metrics, B metrics, deltas
    ...
    st.download_button("Download CSV — Comparison (A vs B)", csv, file_name="performance_comparison_A_vs_B.csv", mime="text/csv")
...
# ===========================
# 📊 Performance ▸ Comparison (Window A vs Window B)
# ===========================
