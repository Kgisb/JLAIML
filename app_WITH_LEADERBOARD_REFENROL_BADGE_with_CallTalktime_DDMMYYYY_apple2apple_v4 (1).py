...
def _ata_pick(df, preferred, variants):
...
def _ata_to_datetime(s):
    # robust coercion with dayfirst True and epoch fallback
...
def _ata_load_main_df():
    DEFAULT = globals().get('DEFAULT_DATA_PATH', 'Master_sheet-DB.csv')
    data_src = _st_ata.session_state.get('data_src', DEFAULT)
    try:
        df = _pd_ata.read_csv(data_src, low_memory=False)
    except Exception:
        df = _pd_ata.DataFrame()
    if df.empty:
        return df
    df.columns = [c.strip() for c in df.columns]
    stg = _ata_pick(df, None, ["Deal Stage","Stage","Pipeline Stage"])
    if stg:
        mask_invalid = df[stg].astype(str).str.contains(r"1\.2\s*Invalid deal", case=False, na=False)
        df = df.loc[~mask_invalid].copy()
    return df

def _ata_metric_map():
    return {
        "Leads (Creates)"        : ["Create Date","Created Date","Deal Create Date","CreateDate","Created On"],
        "Enrolments"             : ["Payment Received Date","Payment Received Date ","Payment Date","Enrolment Date","PaymentReceivedDate","Paid On"],
        ...
        "Referral Deals"         : ["Create Date","Created Date","Deal Create Date","CreateDate","Created On"],
        "Referral Enrolments"    : ["Payment Received Date","Payment Received Date ","Payment Date","Enrolment Date","PaymentReceivedDate","Paid On"],
    }

def _ata_resolve_col(df, variants):
    for v in variants:
...
def _ata_totals_by_source(df, metric, start_date, end_date):
    src_col = _ata_pick(df, None, ["JetLearn Deal Source","Deal Source","Source"])
    if src_col is None:
        return _pd_ata.DataFrame(columns=["source","value"])

    date_col = _ata_resolve_col(df, _ata_metric_map().get(metric, []))
    if date_col is None:
        return _pd_ata.DataFrame(columns=["source","value"])

    s = _ata_to_datetime(df[date_col])
    mask = s.notna() & (s.dt.date >= start_date) & (s.dt.date <= end_date)
    ...
    if metric.startswith("Referral"):
        ...
        if metric == "Referral Enrolments":
            pay_col = _ata_resolve_col(dfx, _ata_metric_map()["Enrolments"])
            if pay_col is not None:
                dfx = dfx[_ata_to_datetime(dfx[pay_col]).notna()]
            else:
                dfx = dfx.iloc[0:0]

    out = (dfx.groupby(src_col, dropna=False).size()
            .reset_index(name="value")
            .rename(columns={src_col: "source"}))
    return out

def _ata_grouped(df, metric, start_date, end_date, granularity):
    src_col = _ata_pick(df, None, ["JetLearn Deal Source","Deal Source","Source"])
    date_col = _ata_resolve_col(df, _ata_metric_map().get(metric, []))
    if src_col is None or date_col is None:
        return _pd_ata.DataFrame(columns=["source","bucket","count"]) 
    ...
    ds = _pd_ata.to_datetime(dfx["__date"], errors="coerce")
    if granularity == "Day":
        dfx["bucket"] = ds.dt.date
    elif granularity == "Week":
        iso = ds.dt.isocalendar()
        dfx["bucket"] = iso["year"].astype(str) + "-W" + iso["week"].astype(str).str.zfill(2)
    else:  # Month
        dfx["bucket"] = ds.dt.to_period("M").astype(str)

    out = (dfx.groupby([src_col, "bucket"], dropna=False).size()
...
def _ata_render():
    ...
    _st_ata.caption("Compare two windows (A vs B) with MTD/Cohort scopes and Day/Week/Month granularity.")

    df = globals().get('df_f', None)
    if df is None or (hasattr(df, 'empty') and df.empty):
        df = _ata_load_main_df()
    if df is None or (hasattr(df, 'empty') and df.empty):
        _st_ata.info("No data available. Check the primary dataset path in the app header.")
        return

    # Global filters (aligned to your DB)
    c0, c1, c2, c3, c4 = _st_ata.columns([1,1,1,1,1])
    counsellor_col = _ata_pick(df, None, ["Student/Academic Counsellor","Counsellor","Academic Counsellor"]) 
    country_col    = _ata_pick(df, None, ["Country","Country Name"]) 
    source_col     = _ata_pick(df, None, ["JetLearn Deal Source","Deal Source","Source"]) 
    pipeline_col   = _ata_pick(df, None, ["Pipeline"]) 
    ...
    if counsellor_col and sel_cons:
        df = df[df[counsellor_col].astype(str).isin(sel_cons)]
    if country_col and sel_cnt:
        df = df[df[country_col].astype(str).isin(sel_cnt)]
    if source_col and sel_src:
        df = df[df[source_col].astype(str).isin(sel_src)]
    if pipeline_col and sel_pipe:
        df = df[df[pipeline_col].astype(str).isin(sel_pipe)]
    ...
    # source selection for display order
    all_sources = sorted(df[source_col].dropna().astype(str).unique().tolist()) if source_col else []
    display_sources = _st_ata.multiselect("Show Sources", options=all_sources, default=all_sources[:12])

    # Totals by source
    totA = _ata_totals_by_source(df, metric, startA, endA)
    totB = _ata_totals_by_source(df, metric, startB, endB)
    ...
    merged = (base.merge(totA.rename(columns={"value":"A"}), on="source", how="left")
                  .merge(totB.rename(columns={"value":"B"}), on="source", how="left"))
    ...
    _st_ata.markdown("### Source-wise KPIs (A vs B)")
    for _, r in merged.iterrows():
        ...
        with c4:
            dp = "–" if _pd_ata.isna(r["Δ%"]) else f"{r['Δ%']:.1f}%"
            _st_ata.metric("Δ (B−A)", int(r["Δ"]), delta=dp)
    ...
    # Trends by granularity (optional)
    _st_ata.markdown("### Trends by Granularity")
    countsA = _ata_grouped(df, metric, startA, endA, granA)
    countsB = _ata_grouped(df, metric, startB, endB, granB)
    cta, ctb = _st_ata.columns(2)
    with cta:
        _st_ata.markdown("**Window A**")
        if not countsA.empty:
            _st_ata.dataframe(countsA, use_container_width=True)
            _st_ata.download_button("⬇️ Download A", data=countsA.to_csv(index=False).encode("utf-8"), file_name="apple_to_apple_windowA.csv", mime="text/csv")
        else:
            _st_ata.info("No rows in A for the chosen filters.")
    with ctb:
        _st_ata.markdown("**Window B**")
        if not countsB.empty:
            _st_ata.dataframe(countsB, use_container_width=True)
            _st_ata.download_button("⬇️ Download B", data=countsB.to_csv(index=False).encode("utf-8"), file_name="apple_to_apple_windowB.csv", mime="text/csv")
        else:
            _st_ata.info("No rows in B for the chosen filters.")
