...
def _render_call_talktime_report():
    ...
    # Pick a single visualization day (defaults to start date used above)
    try:
        default_vis_date = d_start
    except Exception:
        default_vis_date = df_win["_dt"].dt.date.min()
    vis_date = st.date_input("Visualization day", value=default_vis_date, min_value=df_win["_dt"].dt.date.min(), max_value=df_win["_dt"].dt.date.max(), key="ctt_vis_day")
    ...
    if df_day.empty:
        ...
        # Aggregate by counsellor (caller) x hour
        ...
        # Download data
        csv_bubble = agg.rename(columns={"_hour":"Hour"}).to_csv(index=False).encode("utf-8")
        st.download_button("⬇️ Download bubble data (counsellor × hour)", csv_bubble, "counsellor_hour_bubble.csv", "text/csv")

        st.dataframe(country_tot[[country_col, "Calls", "Total Talk Time", "Total Seconds"]], use_container_width=True)
        ...
        st.dataframe(country_tot_gt60[[country_col, "Calls (>60s)", "Total Talk Time (>60s)", "Total Seconds (>60s)"]], use_container_width=True)
