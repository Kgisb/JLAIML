...
def _render_call_talktime_report():
    ...
    vis_date = st.date_input(
        "Visualization day",
        value=default_vis_date,
        min_value=df_win["_dt"].dt.date.min(),
        max_value=df_win["_dt"].dt.date.max(),
        key="ctt_vis_day"
    )

    df_day = df_win[df_win["_dt"].dt.date == vis_date].copy()
    if df_day.empty:
        st.info("No calls on the selected day.")
    else:
        df_day["_hour"] = df_day["_dt"].dt.hour

        # Aggregate by Counsellor × Hour
        agg_calls = df_day.groupby([caller_col, "_hour"], dropna=False).size().reset_index(name="Calls")
        agg_secs  = df_day.groupby([caller_col, "_hour"], dropna=False)["_secs"].sum().reset_index(name="Total Seconds")
        ...
        per_caller_hour = df_day.groupby([caller_col, "_hour"], dropna=False)["_secs"].sum().reset_index(name="Total Seconds")
        ...
        callers = sorted(df_day[caller_col].dropna().astype(str).unique().tolist())
