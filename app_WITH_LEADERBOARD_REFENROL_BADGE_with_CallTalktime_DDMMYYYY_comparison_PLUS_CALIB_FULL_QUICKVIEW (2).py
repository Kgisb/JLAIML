...
def _render_performance_quick_view(
    ...
    *,
    ...
    # ---- 0) Fail-fast if current filters return no rows
    if df_f is None or len(df_f) == 0:
        st.info("No rows in current selection. Try widening filters in the drawer to see data here.")
        return

    # -------- robust column resolver
    def _pick_col(df: pd.DataFrame, candidates, substr_candidates=None):
        # exact
        for c in candidates:
            if c in df.columns: return c
        ...
        if substr_candidates:
            for sub in substr_candidates:
                for col in df.columns:
                    if sub.lower() in col.lower():
                        return col
        return None

    # Try to auto-correct incoming names when needed
    _create_in = create_col if create_col in df_f.columns else _pick_col(df_f, [create_col, "Create Date"], substr_candidates=["create"])
    _pay_in    = pay_col    if pay_col    in df_f.columns else _pick_col(df_f, [pay_col, "Payment Received Date", "Payment Received Date "], substr_candidates=["payment","enrol"])
    _src_in    = source_col if (source_col and source_col in df_f.columns) else _pick_col(df_f, ["JetLearn Deal Source","Deal Source"], substr_candidates=["source"])
    _ac_in     = _pick_col(df_f, ["Academic Counsellor","Student/Academic Counsellor","AC","Counsellor","Sales Owner","Lead Owner","Owner","Advisor","Agent"], substr_candidates=["counsell","owner"])
    _country_in= _pick_col(df_f, ["Country","Country Name","Geo","Region"], substr_candidates=["country","region","geo"])
    _first_in  = first_cal_sched_col if (first_cal_sched_col and first_cal_sched_col in df_f.columns) else _pick_col(df_f, ["First Calibration Scheduled Date","First Calibration","First Cal Scheduled"], substr_candidates=["first cal","first calibration"])
    _resch_in  = cal_resched_col     if (cal_resched_col     and cal_resched_col     in df_f.columns) else _pick_col(df_f, ["Calibration Reschedule Date","Cal Reschedule","Rescheduled Date"], substr_candidates=["resched"])
    _done_in   = cal_done_col        if (cal_done_col        and cal_done_col        in df_f.columns) else _pick_col(df_f, ["Calibration Done Date","Cal Done","Trial Done","Trial Completed"], substr_candidates=["done","completed","trial"])

    # Diagnostics expander so you can see what columns got detected
    with st.expander("Quick View • Data diagnostics", expanded=False):
        st.write("Detected columns:")
        st.write({
            "create": _create_in,
            "pay": _pay_in,
            "source": _src_in,
            "academic_counsellor": _ac_in,
            "country": _country_in,
            "first_cal": _first_in,
            "reschedule": _resch_in,
            "done": _done_in,
            "rows_in_scope": len(df_f),
        })
        st.caption("If any of these show **None**, tell me the exact column name(s) and I will pin them.")

    # ---- guards: ensure we can compute at least create/pay; otherwise show guidance
    if _create_in is None and _pay_in is None:
        st.error("Couldn't detect Deal Create or Payment/Enrolment date columns in the current data.", icon="⚠️")
        st.stop()
    ...
    d = df_f.copy()
    d["__create"] = _dt(d[_create_in]) if _create_in in d.columns else pd.NaT
    d["__pay"]    = _dt(d[_pay_in]) if _pay_in in d.columns else pd.NaT
    d["__src"]    = (d[_src_in].fillna("Unknown").astype(str) if _src_in in d.columns else pd.Series(["Unknown"]*len(d), index=d.index))
    d["__ac"]     = (d[_ac_in].fillna("Unknown").astype(str) if _ac_in in d.columns else pd.Series(["Unknown"]*len(d), index=d.index))
    d["__country"]= (d[_country_in].fillna("Unknown").astype(str) if _country_in in d.columns else pd.Series(["Unknown"]*len(d), index=d.index))

    # Optional calibration dates
    d["__first"] = _dt(d[_first_in]) if (_first_in and _first_in in d.columns) else pd.NaT
    d["__resch"] = _dt(d[_resch_in]) if (_resch_in and _resch_in in d.columns) else pd.NaT
    d["__done"]  = _dt(d[_done_in])  if (_done_in  and _done_in  in d.columns) else pd.NaT

    # --- derive max date from data to avoid blank "today/yesterday" when dataset is historical
    data_dates = pd.concat([d["__create"].dropna(), d["__pay"].dropna()])
    if len(data_dates):
        latest_data_day = data_dates.max().date()
        month_start_default = latest_data_day.replace(day=1)
    else:
        latest_data_day = date.today()
        month_start_default = date.today().replace(day=1)

    # Controls for "today/yesterday" based on latest data
    use_latest = st.toggle("Use latest data day for Today/Yesterday defaults", value=True)

    base_today = latest_data_day if use_latest else date.today()
    today = base_today
    yday  = base_today - timedelta(days=1)

    # Helper to build window summary with MTD/Cohort logic
    ...
    cA, cB, cC = st.columns(3)
    with cA:
        rng_start = st.date_input("Start date", value=month_start_default, key="qv_start")
    with cB:
        rng_end = st.date_input("End date", value=latest_data_day, key="qv_end")
    with cC:
    ...
    def _section_tables(df_source: pd.DataFrame, title: str, group_key: str | None, label_suffix: str, display_name: str | None = None):
        with st.container():
            ...
            if len(df_source) == 0:
                c1.info("No rows here."); c2.empty(); c3.empty(); return
    ...
    with tab_ac:
        _section_tables(d_y, "## Yesterday", group_key="__ac", label_suffix="(yesterday)", display_name=_ac_in or "Academic Counsellor")
        st.divider()
        _section_tables(d_t, "## Today", group_key="__ac", label_suffix="(today)", display_name=_ac_in or "Academic Counsellor")
        st.divider()
        _section_tables(d_r, "## Range", group_key="__ac", label_suffix="(range)", display_name=_ac_in or "Academic Counsellor")

    with tab_ctry:
        _section_tables(d_y, "## Yesterday", group_key="__country", label_suffix="(yesterday)", display_name=_country_in or "Country")
        st.divider()
        _section_tables(d_t, "## Today", group_key="__country", label_suffix="(today)", display_name=_country_in or "Country")
        st.divider()
        _section_tables(d_r, "## Range", group_key="__country", label_suffix="(range)", display_name=_country_in or "Country")
    ...
    try:
        ...
        st.warning(f"Chart couldn't render: {e}")
