    ...
    *,
    ...
    def _pick_col(df: pd.DataFrame, candidates):
        for c in candidates:
            if c in df.columns:
                return c
        # fuzzy match (case-insensitive, strip)
    ...
    ac_candidates = [
        "Academic Counsellor","AC","Counsellor","Sales Owner","Lead Owner","Owner","Advisor","Agent"
    ]
    country_candidates = [
        "Country","Country Name","Geo","Region"
    ]
    ...
    if source_col is None or source_col not in df_f.columns:
        ...
        st.warning("JetLearn Deal Source column not found; grouping will show 'Unknown'.", icon="⚠️")
    else:
        src_series = df_f[source_col].fillna("Unknown").astype(str).rename("__src__")

    if ac_col_res is None:
        ac_series = pd.Series(["Unknown"] * len(df_f), index=df_f.index, name="__ac__")
    else:
        ac_series = df_f[ac_col_res].fillna("Unknown").astype(str).rename("__ac__")

    if country_col_res is None:
        country_series = pd.Series(["Unknown"] * len(df_f), index=df_f.index, name="__country__")
    else:
        country_series = df_f[country_col_res].fillna("Unknown").astype(str).rename("__country__")
    ...
    # Yesterday (mode independent)
    d_y = _prep_window(yday, yday, mode=None)
    # Today (mode independent)
    ...
    def _section_tables(df_source: pd.DataFrame, title: str, group_key: str | None, label_suffix: str):
        with st.container():
            st.markdown(title)
            ...
            if group_key is None:
                ...
                with c1:
                    bx1 = (
                        df_source.groupby(group_key, dropna=False)["Deals_Created"]
                                 .sum()
                                 .reset_index()
                                 .rename(columns={group_key: group_key.strip("_").strip("__").title().replace("_"," "), 
                                                  "Deals_Created": f"Deals Created {label_suffix}"})
                    )
                ...
                with c2:
                    ...
                    bx2 = bx2.rename(columns={group_key: group_key.strip("_").strip("__").title().replace("_"," "),
                ...
                with c3:
                    bx3 = (
                        df_source.groupby(group_key, dropna=False)["Enrolments"]
                                 ...
                                 .reset_index()
                                 .rename(columns={group_key: group_key.strip("_").strip("__").title().replace("_"," "), 
                                                  "Enrolments": f"Enrolments {label_suffix}"})
                    )
    ...
    with tab_src:
        _section_tables(d_y, "## Yesterday", group_key="__src", label_suffix="(yesterday)")
        st.divider()
        _section_tables(d_t, "## Today", group_key="__src", label_suffix="(today)")
        st.divider()
        _section_tables(d_r, "## Range", group_key="__src", label_suffix="(range)")

    with tab_ac:
        _section_tables(d_y, "## Yesterday", group_key="__ac", label_suffix="(yesterday)")
        st.divider()
        _section_tables(d_t, "## Today", group_key="__ac", label_suffix="(today)")
        st.divider()
        _section_tables(d_r, "## Range", group_key="__ac", label_suffix="(range)")

    with tab_ctry:
        _section_tables(d_y, "## Yesterday", group_key="__country", label_suffix="(yesterday)")
        st.divider()
        _section_tables(d_t, "## Today", group_key="__country", label_suffix="(today)")
        st.divider()
        _section_tables(d_r, "## Range", group_key="__country", label_suffix="(range)")
