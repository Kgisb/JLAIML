...
def _ata_totals_by_source(df, metric, start_date, end_date):
    ...
    # map metric to a date column family
    metric_map = {
    ...
    date_col = None
    for v in metric_map.get(metric, []):
        if v in df.columns:
            date_col = v; break
    if date_col is None:
        # fuzzy contains
    ...
    if metric.startswith("Referral"):
        ...
        if metric == "Referral Enrolments":
            # ensure has payment date as well
            pay_col = _ata_pick(dfx, None, ["Payment Received Date","Payment Received Date ","Payment Date","Enrolment Date","PaymentReceivedDate","Paid On"])
...
def _ata_window_ui(label):
    today = _dt_ata.date.today()
    first_of_month = today.replace(day=1)
    mode = _st_ata.selectbox(f"{label} Mode", ["MTD","Cohort","Custom"], index=0, key=f"ata_mode_{label}")
    if mode == "MTD":
        start = _st_ata.date_input(f"{label} Start", first_of_month, key=f"ata_start_{label}")
        end   = _st_ata.date_input(f"{label} End", today, key=f"ata_end_{label}")
    elif mode == "Cohort":
        last_month_end = (first_of_month - _dt_ata.timedelta(days=1))
        last_month_start = last_month_end.replace(day=1)
        start = _st_ata.date_input(f"{label} Cohort Start", last_month_start, key=f"ata_start_{label}")
        end   = _st_ata.date_input(f"{label} Cohort End", last_month_end, key=f"ata_end_{label}")
    else:
        start = _st_ata.date_input(f"{label} Custom Start", first_of_month, key=f"ata_start_{label}")
        end   = _st_ata.date_input(f"{label} Custom End", today, key=f"ata_end_{label}")
    if start > end:
        _st_ata.error(f"{label}: Start date cannot be after End date.")
    return mode, start, end

def _ata_render():
    _st_ata.subheader("Performance — Apple-to-Apple Comparison")
    _st_ata.caption("Compare two time windows across selected JetLearn Deal Sources for a chosen metric.")
    ...
    # Basic global filters (aligned with common columns in your DB)
    ...
    _st_ata.markdown("### Select Time Windows")
    _, (modeA, startA, endA), (modeB, startB, endB), _ = _st_ata.columns([.2,.3,.3,.2])
    with _st_ata.columns([.2,.3,.3,.2])[1]:
        modeA, startA, endA = _ata_window_ui("Window A")
    with _st_ata.columns([.2,.3,.3,.2])[2]:
        modeB, startB, endB = _ata_window_ui("Window B")

    # Metric selection
    metric = _st_ata.selectbox(
        "Metric",
        ["Leads (Creates)","Enrolments","First Cal Scheduled","Cal Rescheduled","Cal Done","Referral Deals","Referral Enrolments"],
        index=0
    )

    # Source selection (display subset)
    all_sources = sorted(df[source_col].dropna().astype(str).unique().tolist()) if source_col else []
    show_sources = _st_ata.multiselect("Show Sources", options=all_sources, default=all_sources[:10])
    ...
    # Universe
    universe = show_sources if show_sources else sorted(set(totA["source"]).union(set(totB["source"])))
    ...
    _st_ata.markdown("### Source-wise Comparison")
    ...
    with _st_ata.expander("Download Summary", expanded=True):
        ...
        _st_ata.download_button(
            "⬇️ Download CSV",
            data=merged.to_csv(index=False).encode("utf-8"),
            file_name="apple_to_apple_summary.csv",
            mime="text/csv"
        )
