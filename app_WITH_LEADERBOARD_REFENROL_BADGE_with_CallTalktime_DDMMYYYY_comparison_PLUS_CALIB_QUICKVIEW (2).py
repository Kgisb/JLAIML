...
with st.sidebar:
    ...
    MASTER_SECTIONS = {
        "Performance": ["Cash-in","Dashboard","MIS","Daily Business","Sales Tracker","AC Wise Detail","Leaderboard", "Quick View", "Comparison", "Call Talk-time Report"],
...
def _render_performance_quick_view(
    ...
    *,
    ...
    # ---- column guards
    if source_col is None or source_col not in df_f.columns:
        st.warning("JetLearn Deal Source column not found.", icon="⚠️")
    ...
    # ---------- (A) Yesterday boxes ----------
    ym_start, ym_end = yday, yday
    m_create_y = d["__create"].dt.date.between(ym_start, ym_end)
    m_pay_y    = d["__pay"].dt.date.between(ym_start, ym_end)
    m_first_y  = pd.to_datetime(d["__first"], errors="coerce").dt.date.between(ym_start, ym_end) if d["__first"].notna().any() else pd.Series(False, index=d.index)
    m_resch_y  = pd.to_datetime(d["__resch"], errors="coerce").dt.date.between(ym_start, ym_end) if d["__resch"].notna().any() else pd.Series(False, index=d.index)
    m_done_y   = pd.to_datetime(d["__done"],  errors="coerce").dt.date.between(ym_start, ym_end)  if d["__done"].notna().any()  else pd.Series(False, index=d.index)

    d_y = d.copy()
    d_y["Deals_Created"] = m_create_y.astype(int)
    d_y["First_Cal"]     = m_first_y.astype(int)
    d_y["Cal_Resched"]   = m_resch_y.astype(int)
    d_y["Cal_Done"]      = m_done_y.astype(int)
    d_y["Enrolments"]    = m_pay_y.astype(int)
    ...
    with st.container():
        ...
        with c1:
            bx1 = (
                d_y.groupby("__src", dropna=False)["Deals_Created"]
                   .sum()
                   .reset_index()
                   .rename(columns={"__src":"JetLearn Deal Source","Deals_Created":"Deals Created (yesterday)"})
            )
            st.dataframe(bx1, use_container_width=True, hide_index=True)
        with c2:
            bx2 = (
                d_y.groupby("__src", dropna=False)[["First_Cal","Cal_Resched","Cal_Done"]]
                   .sum()
                   .reset_index()
                   .rename(columns={"__src":"JetLearn Deal Source","First_Cal":"First Cal (y)","Cal_Resched":"Cal Resch (y)","Cal_Done":"Cal Done (y)"})
            )
            st.dataframe(bx2, use_container_width=True, hide_index=True)
        with c3:
            bx3 = (
                d_y.groupby("__src", dropna=False)["Enrolments"]
                   .sum()
                   .reset_index()
                   .rename(columns={"__src":"JetLearn Deal Source","Enrolments":"Enrolments (yesterday)"})
            )
    ...
    # ---------- (B) Today boxes ----------
    tm_start, tm_end = today, today
    m_create_t = d["__create"].dt.date.between(tm_start, tm_end)
    m_pay_t    = d["__pay"].dt.date.between(tm_start, tm_end)
    m_first_t  = pd.to_datetime(d["__first"], errors="coerce").dt.date.between(tm_start, tm_end) if d["__first"].notna().any() else pd.Series(False, index=d.index)
    m_resch_t  = pd.to_datetime(d["__resch"], errors="coerce").dt.date.between(tm_start, tm_end) if d["__resch"].notna().any() else pd.Series(False, index=d.index)
    m_done_t   = pd.to_datetime(d["__done"],  errors="coerce").dt.date.between(tm_start, tm_end)  if d["__done"].notna().any()  else pd.Series(False, index=d.index)

    d_t = d.copy()
    d_t["Deals_Created"] = m_create_t.astype(int)
    d_t["First_Cal"]     = m_first_t.astype(int)
    d_t["Cal_Resched"]   = m_resch_t.astype(int)
    d_t["Cal_Done"]      = m_done_t.astype(int)
    d_t["Enrolments"]    = m_pay_t.astype(int)
    ...
    with st.container():
        ...
        with c1:
            bx1 = (
                d_t.groupby("__src", dropna=False)["Deals_Created"]
                   .sum()
                   .reset_index()
                   .rename(columns={"__src":"JetLearn Deal Source","Deals_Created":"Deals Created (today)"})
            )
            st.dataframe(bx1, use_container_width=True, hide_index=True)
        with c2:
            bx2 = (
                d_t.groupby("__src", dropna=False)[["First_Cal","Cal_Resched","Cal_Done"]]
                   .sum()
                   .reset_index()
                   .rename(columns={"__src":"JetLearn Deal Source","First_Cal":"First Cal (t)","Cal_Resched":"Cal Resch (t)","Cal_Done":"Cal Done (t)"})
            )
            st.dataframe(bx2, use_container_width=True, hide_index=True)
        with c3:
            bx3 = (
                d_t.groupby("__src", dropna=False)["Enrolments"]
                   .sum()
                   .reset_index()
                   .rename(columns={"__src":"JetLearn Deal Source","Enrolments":"Enrolments (today)"})
            )
            st.dataframe(bx3, use_container_width=True, hide_index=True)

    st.divider()

    # ---------- (C) Range boxes (MTD/Cohort switch) ----------
    ...
    m_create = d["__create"].dt.date.between(rng_start, rng_end)
    m_pay    = d["__pay"].dt.date.between(rng_start, rng_end)
    m_first  = pd.to_datetime(d["__first"], errors="coerce").dt.date.between(rng_start, rng_end) if d["__first"].notna().any() else pd.Series(False, index=d.index)
    m_resch  = pd.to_datetime(d["__resch"], errors="coerce").dt.date.between(rng_start, rng_end) if d["__resch"].notna().any() else pd.Series(False, index=d.index)
    m_done   = pd.to_datetime(d["__done"],  errors="coerce").dt.date.between(rng_start, rng_end)  if d["__done"].notna().any()  else pd.Series(False, index=d.index)
    ...
    d_r = d.copy()
    d_r["Deals_Created"] = m_create.astype(int)
    d_r["First_Cal"]     = first_mask.astype(int)
    d_r["Cal_Resched"]   = resch_mask.astype(int)
    d_r["Cal_Done"]      = done_mask.astype(int)
    d_r["Enrolments"]    = enrol_mask.astype(int)

    r1, r2, r3 = st.columns(3)
    with r1:
        deals_box = (
            d_r.groupby("__src", dropna=False)["Deals_Created"]
               .sum()
               .reset_index()
               .rename(columns={"__src":"JetLearn Deal Source","Deals_Created":"Deals Created (range)"})
        )
        st.dataframe(deals_box, use_container_width=True, hide_index=True)

    with r2:
        cal_box = (
            d_r.groupby("__src", dropna=False)[["First_Cal","Cal_Resched","Cal_Done"]]
               .sum()
               .reset_index()
               .rename(columns={"__src":"JetLearn Deal Source",
                                "First_Cal":"First Cal (range)",
                                "Cal_Resched":"Cal Resch (range)",
                                "Cal_Done":"Cal Done (range)"}))
        st.dataframe(cal_box, use_container_width=True, hide_index=True)

    with r3:
        en_box = (
            d_r.groupby("__src", dropna=False)["Enrolments"]
               .sum()
               .reset_index()
               .rename(columns={"__src":"JetLearn Deal Source","Enrolments":"Enrolments (range)"})
        )
        st.dataframe(en_box, use_container_width=True, hide_index=True)
    ...
    # Build day series on the same range
    base_start = rng_start
    base_end   = rng_end
    days = pd.date_range(base_start, base_end, freq="D").date

    # Deals = count of create per day (post referral-exclusion)
    d_series = (
    ...
    # Enrolments line = (mode consistent with boxes)
    if mode == "MTD":
...
# --- Performance / Quick View (Injected)
...
# ====== Quick View Renderer (Overridden with Overall toggle) ======
def _render_performance_quick_view(
    df_f: pd.DataFrame,
    *,
    create_col: str,
    ...
    st.subheader("Performance — Quick View")
    overall_mode = st.checkbox("Show Overall totals (sum across sources)", value=False, key="qv_overall")
    ...
    with st.container():
        ...
        with c1:
            if overall_mode:
                total = int(d_y["Deals_Created"].sum())
                st.dataframe(pd.DataFrame({"Deals Created (yesterday)":[total]}), use_container_width=True, hide_index=True)
            else:
                bx1 = (
                    d_y.groupby("__src", dropna=False)["Deals_Created"]
                       .sum()
        ...
        with c2:
            if overall_mode:
                totals = {
                    "First Cal (y)" : int(d_y["First_Cal"].sum()),
                    "Cal Resch (y)" : int(d_y["Cal_Resched"].sum()),
                    "Cal Done (y)"  : int(d_y["Cal_Done"].sum()),
                }
        ...
        with c3:
            if overall_mode:
                total = int(d_y["Enrolments"].sum())
                st.dataframe(pd.DataFrame({"Enrolments (yesterday)":[total]}), use_container_width=True, hide_index=True)
            else:
                bx3 = (
                    d_y.groupby("__src", dropna=False)["Enrolments"]
                       .sum()
    ...
    with st.container():
        ...
        with c1:
            if overall_mode:
                total = int(d_t["Deals_Created"].sum())
                st.dataframe(pd.DataFrame({"Deals Created (today)":[total]}), use_container_width=True, hide_index=True)
            else:
                bx1 = (
                    d_t.groupby("__src", dropna=False)["Deals_Created"]
                       .sum()
        ...
        with c2:
            if overall_mode:
                totals = {
                    "First Cal (t)" : int(d_t["First_Cal"].sum()),
                    "Cal Resch (t)" : int(d_t["Cal_Resched"].sum()),
                    "Cal Done (t)"  : int(d_t["Cal_Done"].sum()),
                }
                st.dataframe(pd.DataFrame([totals]), use_container_width=True, hide_index=True)
            else:
                bx2 = (
        ...
        with c3:
            if overall_mode:
                total = int(d_t["Enrolments"].sum())
                st.dataframe(pd.DataFrame({"Enrolments (today)":[total]}), use_container_width=True, hide_index=True)
            else:
                bx3 = (
                    d_t.groupby("__src", dropna=False)["Enrolments"]
                       .sum()
    ...
    with r1:
        if overall_mode:
            total = int(d_r["Deals_Created"].sum())
            st.dataframe(pd.DataFrame({"Deals Created (range)":[total]}), use_container_width=True, hide_index=True)
        else:
            deals_box = (
                d_r.groupby("__src", dropna=False)["Deals_Created"]
                   .sum()
    ...
    with r2:
        if overall_mode:
            totals = {
                "First Cal (range)" : int(d_r["First_Cal"].sum()),
                "Cal Resch (range)" : int(d_r["Cal_Resched"].sum()),
                "Cal Done (range)"  : int(d_r["Cal_Done"].sum()),
            }
            st.dataframe(pd.DataFrame([totals]), use_container_width=True, hide_index=True)
        else:
            cal_box = (
    ...
    with r3:
        if overall_mode:
            total = int(d_r["Enrolments"].sum())
            st.dataframe(pd.DataFrame({"Enrolments (range)":[total]}), use_container_width=True, hide_index=True)
        else:
            en_box = (
                d_r.groupby("__src", dropna=False)["Enrolments"]
                   .sum()
