...
def _ata_metric_totals(df, metric, start, end, *, mode, create_col=None, pay_col=None):
    ...
    _create = create_col or _ata_pick(df, metric_map["Leads (Creates)"])
    _pay    = pay_col    or _ata_pick(df, metric_map["Enrolments"])
    date_col = _ata_pick(df, metric_map.get(metric, []))

    if src_col is None or date_col is None or _create is None or _pay is None:
        return _pd_ata.DataFrame(columns=["source","value"])

    # Normalize series
    C = _ata_to_datetime(df[_create]).dt.date
    P = _ata_to_datetime(df[_pay]).dt.date
    S = _ata_to_datetime(df[date_col]).dt.date

    # base masks by chosen window (start..end)
    m_created = (C >= start) & (C <= end)
    m_paid    = (P >= start) & (P <= end)
    m_metric  = (S >= start) & (S <= end)

    # Mode (MTD/Cohort) rule mirrors MIS:
    def effective_mask(metric_name):
        if metric_name in ["Leads (Creates)", "Referral Deals"]:
            return m_created
        elif metric_name in ["Enrolments", "Referral Enrolments"]:
            return (m_paid & (m_created if mode == "MTD" else True))
        elif metric_name in ["First Cal Scheduled", "Cal Rescheduled", "Cal Done"]:
            return (m_metric & (m_created if mode == "MTD" else True))
        else:
            return m_metric

    eff = effective_mask(metric)
    ...
    if metric.startswith("Referral"):
        ...
        if metric == "Referral Enrolments":
            P_eff = _ata_to_datetime(dfx[_pay]).dt.date
            dfx = dfx[(P_eff >= start) & (P_eff <= end)]
            if mode == "MTD":
                C_eff = _ata_to_datetime(dfx[_create]).dt.date
                dfx = dfx[(C_eff >= start) & (C_eff <= end)]

    out = (dfx.assign(**{src_col: dfx[src_col].fillna("Unknown").astype(str)})
              .groupby(src_col, dropna=False).size()
...
def _ata_render():
    ...
    # Totals
    totA = _ata_metric_totals(f, metric, startA, endA, mode=modeA, create_col=create_col if 'create_col' in globals() else None, pay_col=pay_col if 'pay_col' in globals() else None)
    totB = _ata_metric_totals(f, metric, startB, endB, mode=modeB, create_col=create_col if 'create_col' in globals() else None, pay_col=pay_col if 'pay_col' in globals() else None)
