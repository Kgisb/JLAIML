...
def _render_call_talktime_report():
    ...
    def _fmt_ddmmyyyy(d):
        try:
            ...
            return ""
    with c1:
        d_start_str = st.text_input("Start date (DDMMYYYY)", value=_fmt_ddmmyyyy(min_d), key="ctt_start_d_str")
    with c2:
        d_end_str = st.text_input("End date (DDMMYYYY)", value=_fmt_ddmmyyyy(max_d), key="ctt_end_d_str")
    with c3:
    ...
    d_start = _ctt_parse_date_ddmmyyyy(d_start_str) or min_d
    d_end   = _ctt_parse_date_ddmmyyyy(d_end_str)   or max_d
    if d_end < d_start:
