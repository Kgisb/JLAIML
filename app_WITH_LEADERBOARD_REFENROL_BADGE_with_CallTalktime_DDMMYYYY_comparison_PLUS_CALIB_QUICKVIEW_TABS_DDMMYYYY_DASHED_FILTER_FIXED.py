...
def _render_call_talktime_report():
    ...
    # robust parser for DD-MM-YYYY or DDMMYYYY
    def _parse_ddmmyyyy_loose(s, fallback):
        import re as _re
        ...
        try:
            ...
            pass
        # Strip non-digits and try compact
        digits = _re.sub(r"\D", "", s)
        if len(digits) == 8:
            try:
                return _dt.strptime(digits, "%d%m%Y").date()
            except Exception:
                return fallback
        return fallback
    ...
    def _fmt_ddmmyyyy(d):
        try:
            ...
            return ""
    with c1:
        d_start_str = st.text_input("Start date (DD-MM-YYYY)", value=_fmt_ddmmyyyy(min_d), key="ctt_start_d_str")
    with c2:
        d_end_str = st.text_input("End date (DD-MM-YYYY)", value=_fmt_ddmmyyyy(max_d), key="ctt_end_d_str")
    with c3:
    ...
    d_start = _parse_ddmmyyyy_loose(d_start_str, min_d)
    d_end   = _parse_ddmmyyyy_loose(d_end_str, max_d)
    if d_end < d_start:
