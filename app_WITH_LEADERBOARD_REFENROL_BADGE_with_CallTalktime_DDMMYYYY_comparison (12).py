...
def _render_performance_comparison(df_f, create_col, pay_col, counsellor_col, country_col, source_col):
    ...
    st.caption(
        "Compare metrics across two independently-filtered windows (A & B) with separate date ranges. "
    ...
    def _agg(df, dims, start, end):
        ...
        for m in metrics:
            if m == "Deals Created":
                ...
                else:  # Cohort: cohort deals (create in window) that ever enrolled
                    cohort = g.loc[m_create]
                    cnt = cohort.assign(_ok=cohort["_pay"].notna()) \
                                .groupby(group_cols, dropna=False)["_ok"] \
                                .sum(min_count=1)
    ...
    for m in metrics:
        ...
        if colA in merged.columns and colB in merged.columns:
            ...
            # %Δ vs A: numeric-safe; when A==0, set NaN
            import numpy as np
            denom = a.to_numpy()
            num = b.to_numpy()
            pct_arr = np.where(denom != 0, (num / denom) * 100.0, np.nan)
            pct = pd.Series(pct_arr, index=merged.index)
            merged[f"% Δ {m} (vs A)"] = pd.to_numeric(pct, errors='coerce').round(1)
    ...
    rank_options = []
    for c in (a_cols + b_cols + d_cols):
        if c in final.columns: rank_options.append(c)
    default_rank = None
    for pref in [f"% Δ Enrollments (vs A)", f"Enrollments (B)", f"Deals Created (B)"]:
        if pref in rank_options:
            default_rank = pref
            break
    if default_rank is None and rank_options:
        default_rank = rank_options[0]
    with c2:
    ...
    if rank_options and rank_by in to_show.columns:
        # For sorting, treat NaN as very small
    ...
    if limit_choice == "Top 10":
        to_show = to_show.head(10)
    elif limit_choice == "Top 15":
        to_show = to_show.head(15)
    ...
    if show_overall and not to_show.empty:
        ...
        overall = pd.DataFrame([{k: "Overall" for k in key_cols}])
        sums = to_show[num_cols].apply(pd.to_numeric, errors="coerce").sum(numeric_only=True)
        # Recompute %Δ columns from summed A/B if present
        to_append = overall.copy()
        for c in num_cols:
            to_append[c] = sums.get(c, 0.0)
        # If both A and B exist for a metric, recompute %Δ from their sums
        for m in metrics:
            ca = f"{m} (A)"; cb = f"{m} (B)"; cp = f"% Δ {m} (vs A)"
            if ca in to_append.columns and cb in to_append.columns and cp in to_append.columns:
                a_sum = pd.to_numeric(to_append.loc[0, ca], errors="coerce")
                b_sum = pd.to_numeric(to_append.loc[0, cb], errors="coerce")
                pct = (b_sum / a_sum * 100.0) if (pd.notna(a_sum) and a_sum != 0) else float("nan")
                to_append.loc[0, cp] = round(pct, 1) if pd.notna(pct) else pct
        to_show = pd.concat([to_show, to_append], ignore_index=True)
