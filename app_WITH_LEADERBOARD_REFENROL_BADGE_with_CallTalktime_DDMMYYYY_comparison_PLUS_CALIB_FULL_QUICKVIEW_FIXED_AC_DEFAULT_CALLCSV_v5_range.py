...
# ---- Default CSV for Call Talk-time Report (auto-prefill) ----
DEFAULT_CALL_TALKTIME_CSV = "/mnt/data/activityFeedReport_downloads641023449ff5870ffa44af631759945314.csv"

def _prefill_call_talktime(uploaded_file):
    """If no file uploaded, try to auto-load the default CSV from disk.
    Returns a file-like object or the original uploaded_file.
    """
    if uploaded_file is not None:
        return uploaded_file
    try:
        if os.path.exists(DEFAULT_CALL_TALKTIME_CSV):
            # Return a fresh binary handle each time to avoid closed handle issues.
            return open(DEFAULT_CALL_TALKTIME_CSV, "rb")
    except Exception:
        pass
    return uploaded_file
...
def _render_call_talktime_report():
    ...
    # Friendly notice if default CSV is being used
    try:
        if upl is not None and hasattr(upl, 'name') and isinstance(upl.name, str):
            _fname = os.path.basename(getattr(upl, 'name', ''))
        else:
            _fname = ''
        if _fname == '' and os.path.exists(DEFAULT_CALL_TALKTIME_CSV):
            st.info("Loaded default Call Talk-time CSV: **" + os.path.basename(DEFAULT_CALL_TALKTIME_CSV) + "**")
    except Exception:
        pass

    upl = _prefill_call_talktime(upl)
    ...
    caller_cnt = df_win.groupby(caller_col, dropna=False).size().reset_index(name="Calls")
    caller_tot = caller_tot.merge(caller_cnt, on=caller_col, how="left")
    ...
    caller_cnt_gt60 = gt60.groupby(caller_col, dropna=False).size().reset_index(name="Calls (>60s)")
    caller_tot_gt60 = caller_tot_gt60.merge(caller_cnt_gt60, on=caller_col, how="left")
    ...
    if country_col:
        country_tot = (df_win.groupby(country_col, dropna=False)["_secs"].sum().reset_index()
                       .rename(columns={"_secs":"Total Seconds"}).sort_values("Total Seconds", ascending=False))
        country_cnt = df_win.groupby(country_col, dropna=False).size().reset_index(name="Calls")
        country_tot = country_tot.merge(country_cnt, on=country_col, how="left")
        ...
        country_tot_gt60 = (gt60.groupby(country_col, dropna=False)["_secs"].sum().reset_index()
                            .rename(columns={"_secs":"Total Seconds (>60s)"}).sort_values("Total Seconds (>60s)", ascending=False))
        country_cnt_gt60 = gt60.groupby(country_col, dropna=False).size().reset_index(name="Calls (>60s)")
        country_tot_gt60 = country_tot_gt60.merge(country_cnt_gt60, on=country_col, how="left")
    ...
    st.dataframe(caller_tot[[caller_col, "Calls", "Total Talk Time", "Total Seconds"]], use_container_width=True)
    ...
    st.dataframe(caller_tot_gt60[[caller_col, "Calls (>60s)", "Total Talk Time (>60s)", "Total Seconds (>60s)"]], use_container_width=True)
    ...
    # ---------- 4) Academic Counsellor — Time-of-Day Bubble ----------
    st.markdown("### 4) Academic Counsellor — Time-of-Day Bubble")
    st.caption("Aggregated over the selected date range; bubble size reflects total Calls/Seconds across the range.")
    # Use the already-selected date range defaults if available
    try:
        default_vis_date = d_start
    except Exception:
        default_vis_date = df_win["_dt"].dt.date.min()
    vis_range = st.date_input(
        "Visualization range",
        value=(default_vis_date, default_vis_date),
        min_value=df_win["_dt"].dt.date.min(),
        max_value=df_win["_dt"].dt.date.max(),
        key="ctt_vis_range"
    )

    vis_start, vis_end = (vis_range if isinstance(vis_range, (list, tuple)) else (vis_range, vis_range))
    df_rng = df_win[(df_win["_dt"].dt.date >= vis_start) & (df_win["_dt"].dt.date <= vis_end)].copy()
    if df_rng.empty:
        st.info("No calls on the selected day.")
    else:
        df_rng["_hour"] = df_rng["_dt"].dt.hour

        # Aggregate by Counsellor × Hour
        agg_calls = df_rng.groupby([caller_col, "_hour"], dropna=False).size().reset_index(name="Calls")
        agg_secs  = df_rng.groupby([caller_col, "_hour"], dropna=False)["_secs"].sum().reset_index(name="Total Seconds")
        agg = agg_calls.merge(agg_secs, on=[caller_col, "_hour"], how="outer").fillna(0)
        agg["Total Talk Time"] = agg["Total Seconds"].map(_ctt_seconds_to_hms)

        # Size metric control
        size_metric = st.radio("Bubble size metric", ["Calls", "Total Seconds"], horizontal=True, key="ctt_bubble_metric")
        metric_col = "Calls" if size_metric == "Calls" else "Total Seconds"

        # Bubble chart: Hour (X) × Counsellor (Y) with size = metric
        chart = (
            alt.Chart(agg)
            .mark_circle()
            .encode(
                x=alt.X("_hour:O", title="Hour of Day (0–23)"),
                y=alt.Y(f"{caller_col}:N", title="Academic Counsellor"),
                size=alt.Size(f"{metric_col}:Q", title=size_metric, scale=alt.Scale(zero=True)),
                tooltip=[caller_col, "_hour", "Calls", "Total Talk Time", "Total Seconds"]
            )
        ...
        per_caller_hour = df_rng.groupby([caller_col, "_hour"], dropna=False)["_secs"].sum().reset_index(name="Total Seconds")
        max_per = per_caller_hour.sort_values(["Total Seconds"]).groupby(caller_col, as_index=False).last()
        min_per = per_caller_hour.sort_values(["Total Seconds"]).groupby(caller_col, as_index=False).first()

        caller_hour_summary = max_per[[caller_col, "_hour", "Total Seconds"]].rename(
            columns={"_hour":"Max Hour", "Total Seconds":"Max Hour Talk Time"}
        ).merge(
            min_per[[caller_col, "_hour", "Total Seconds"]].rename(
                columns={"_hour":"Min Hour", "Total Seconds":"Min Hour Talk Time"}
            ),
            on=caller_col, how="outer"
        )
        ...
        st.markdown("**Max/Min Hour per Counsellor (by Talk-time)**")
        st.dataframe(
            caller_hour_summary[[caller_col, "Max Hour", "Max Hour Talk Time (HH:MM:SS)", "Min Hour", "Min Hour Talk Time (HH:MM:SS)"]],
            use_container_width=True
        )
        st.download_button(
            "⬇️ Download Caller Hour Summary",
            caller_hour_summary.to_csv(index=False).encode("utf-8"),
            "caller_hour_summary.csv",
            "text/csv"
        )

        # Focused hourly profile for a single counsellor
        callers = sorted(df_rng[caller_col].dropna().astype(str).unique().tolist())
        focus = st.selectbox("Focus: Counsellor hour profile", ["(All)"] + callers, index=0, key="ctt_focus")
        if focus != "(All)":
            ...
            st.download_button(
                f"⬇️ Download Hourly Profile — {focus}",
                foc.to_csv(index=False).encode("utf-8"),
                f"hourly_profile_{focus}.csv",
                "text/csv"
            )
...
def _render_performance_quick_view(
    ...
    *,
    ...
    ac_candidates = ["Counselor", "Student/Academic Counselor", "Student/Academic Counsellor", "Academic Counsellor","AC","Counsellor","Sales Owner","Lead Owner","Owner","Advisor","Agent"]
